...
package body Flyology.Postgres.SQL.C_Oracle is
   ...
   procedure Decode
     (Tree : in out Syntax_Tree; Version : Major_Version;
      Data : System.Address; Length : Natural) is
   begin
      case Version is
   ...
   begin
      ...
      declare
         Error_Text : constant Unbounded_String :=
           Backends.Error_Message (Backend);
      begin
         if Length (Error_Text) /= 0 then
            Result.Parse_Error :=
              (Text => Error_Text,
               Position => Backends.Error_Position (Backend));
      ...
      if Backends.Byte_Length (Backend) = 0 then
         raise Parser_Backend_Error with
           "PostgreSQL C oracle returned neither data nor a diagnostic";
      end if;
      Decode
        (Result, Version, Backends.Data (Backend),
         Backends.Byte_Length (Backend));
      Backends.Release (Backend);
      Result.Valid := True;
   exception
      when Error : Decoders.Decoder_Error =>
         Backends.Release (Backend);
         raise Parser_Backend_Error with
           "malformed C-oracle protobuf result: " &
