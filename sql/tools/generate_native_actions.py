...
def generate_support(
    major: int, helper_entries: list[tuple[dict, Emitter]]
) -> tuple[str, str]:
    unit = f"Flyology.Postgres.SQL.Native.Actions_V{major}_Support"
    spec_lines = [
    ...
    for index, (helper, helper_emitter) in enumerate(helper_entries, 1):
        body_lines.extend(helper_emitter.helper(index, helper))
    body_lines.extend([
        "   function Invoke",
        "     (Build : not null access Builders.Builder; Name : String;",
        "      Arguments : Builders.Semantic_Array) return Builders.Dynamic_Value",
    ...
    for index, (helper, _helper_emitter) in enumerate(helper_entries, 1):
        ...
        body_lines.append(f'      {keyword} Name = "{helper.get("name", "")}" then')
        body_lines.append(f"         return Helper_{index} (Build, Arguments);")
    if helper_entries:
        body_lines.extend([
            "      else",
        ...
        body_lines.append("      return Semantics.Invoke (Build, Name, Arguments);")
    body_lines.extend([
        "   end Invoke;",
        "",
        f"end {unit};",
...
def generate_chunk(
    major: int, number: int, cases: list[tuple[int, list[str]]]
) -> tuple[str, str]:
    unit = f"Flyology.Postgres.SQL.Native.Actions_V{major}_Chunk_{number:02}"
    ...
    body_lines = [
        ...
        "     (Build : not null access Builders.Builder; Name : String;",
        "      Arguments : Builders.Semantic_Array) return Builders.Dynamic_Value",
        f"      renames Actions_V{major}_Support.Invoke;",
        "",
        "   procedure Reduce",
        *reduce_parameters(),
        "   is",
    ...
    for _rule, case_lines in cases:
        body_lines.extend(case_lines)
    body_lines.extend([
        "         when others => null;",
        "      end case;",
        "   end Reduce;",
...
def generate_facade(major: int, chunks: list[list[tuple[int, list[str]]]]) -> str:
    ...
    lines.extend(
        f"with Flyology.Postgres.SQL.Native.Actions_V{major}_Chunk_{number:02};"
    ...
    for number, chunk in enumerate(chunks, 1):
        ...
        lines.extend([
            f"         when {lower} .. {upper} =>",
            f"            Actions_V{major}_Chunk_{number:02}.Reduce",
...
def generate(
    ...
    emitted_cases = [(rule, emitter.case(rule, body)) for rule, body in cases]
    chunks = line_balanced_chunks(emitted_cases, ACTION_CHUNK_COUNT)
    support_spec, support_body = generate_support(major, helper_entries)
    files = {
        f"flyology-postgres-sql-native-actions_v{major}.ads": generate_spec(major),
        f"flyology-postgres-sql-native-actions_v{major}.adb": generate_facade(major, chunks),
        f"flyology-postgres-sql-native-actions_v{major}_support.ads": support_spec,
        f"flyology-postgres-sql-native-actions_v{major}_support.adb": support_body,
    }
    for number, chunk in enumerate(chunks, 1):
        chunk_spec, chunk_body = generate_chunk(major, number, chunk)
        base = f"flyology-postgres-sql-native-actions_v{major}_chunk_{number:02}"
    ...
    return files, unsupported, examples
...
def main() -> None:
    ...
    files, unsupported, examples = generate(args.major, args.vendor.resolve())
    expected = {args.output / name: contents for name, contents in files.items()}
    shard_pattern = f"flyology-postgres-sql-native-actions_v{args.major}_chunk_*"
    actual_shards = {
        path for path in args.output.glob(shard_pattern)
        if path.suffix in {".ads", ".adb"}
    }
    expected_shards = {path for path in expected if "_chunk_" in path.name}
    if args.audit:
    ...
    if args.check:
        ...
        stale.extend(sorted(actual_shards - expected_shards))
        if stale:
            raise SystemExit(
                "generated files are stale: " + ", ".join(str(path) for path in stale)
            )
    else:
        for path in actual_shards - expected_shards:
            path.unlink()
